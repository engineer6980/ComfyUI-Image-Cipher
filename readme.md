...
这是一个用于图像隐私保护的 ComfyUI 自定义节点。它通过 **RGB 通道独立差分位移**算法，将原始图像加密为看似杂乱的彩色噪点图（雪花屏），但保持图像尺寸和 PNG 格式不变。

## ✨ 特点 (Features)
- **视觉不可见**：将 R/G/B 通道拆分并进行不同程度的位移，肉眼无法识别原图内容。
